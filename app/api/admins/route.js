...
export async function GET(request) {
    ...
    const [admins] = await connection.execute('SELECT * FROM finest_admin');
...
export async function POST(request) {
    ...
    const connection = await createConnection();
    const [result] = await connection.execute('INSERT INTO finest_admin (username, password) VALUES (?, ?)', [username, password]);
    await connection.end();
